...
# Add malicious data to affect the model training process 
# and ultimately reduce its accuracy in the prediction phase.
# generate 5 points, equivalent to 5% of the training set
num_chaff = 5
chaff_X = np.array([np.linspace(-2,-1,num_chaff), np.linspace(0.1,0.1, num_chaff)]).T
chaff_y = np.ones(num_chaff)
# Use scikit-learning's partial_fit() API for incremental learning
